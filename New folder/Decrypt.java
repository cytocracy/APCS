...
public class Decrypt extends ConsoleProgram
{
    ...
    public void run()
    {
        ...
        for(int i=0; i<actual.size(); i++){
            ...
            /*for(int j=0; j<actual.get(i).length()-1; j++){
            if(alphabet.indexOf(actual.get(i).substring(j, j+1)) <11){
            actual.remove(i);
            i--;
            break;
        ...
        /*
        while(true){int which = readInt("1 for encrypt 2 for decrypt");
        if(which == 1){
        cr.baseNToBase10();
        } else cr.base10ToBaseN();
        }
         */
