...
public class Picture extends SimplePicture 
{
    ...
    public void insert(Picture p, int row, int col){
        Pixel[][] bigPixels = getPixels2D();
        Pixel[][] smallPixels = p.getPixels2D();
        
        for(int r=0; r<smallPixels.length; r++){
            for(int c=0; c<smallPixels[0].length; c++){
                if(row+r >=0 && row+r < bigPixels.length && col+c >=0 && col+c < bigPixels[0].length && smallPixels[r][c].colorDistance(Color.green) > 100)
                    bigPixels[row+r][col+c].setColor(smallPixels[r][c].getColor());
            }
        }
    }
    public void mirror(){
