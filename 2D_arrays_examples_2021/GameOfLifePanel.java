...
{  
    ...
    private int numNeighbors(int r, int c)
    {
        ...
        if(r!= NUM_ROWS-1 && gameGrid[r+1][c]) counter++;
        if(c!=0 && gameGrid[r][c-1]) counter++;
        if(c!=NUM_COLS-1 && gameGrid[r][c+1]) counter++;
        if(c!=0 && r!=0 && gameGrid[r-1][c-1]) counter++;
        if(r!= 0 && c!= NUM_COLS-1 && gameGrid[r-1][c+1]) counter++;
        if(r!=NUM_ROWS-1 && c!= 0 && gameGrid[r+1][c-1]) counter++;
        if(r!=NUM_ROWS-1 && c!= NUM_COLS-1 && gameGrid[r+1][c+1]) counter++;
    ...
    {         
        ...
        for(int r=0; r<gameGrid.length; r++){
            for(int c=0; c<gameGrid[0].length; c++){
                ...
                if (gameGrid[r][c]){
                    if(n <=1 || n>=4) nextGeneration[r][c] = false;
                    else nextGeneration[r][c] = true;
                } else {
                    if( n==3) nextGeneration[r][c] = true;
                    else nextGeneration[r][c] = false;
                }
            }
        }
