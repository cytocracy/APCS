...
public class Tiles extends GraphicsProgram
{
    ...
    {
        Color color = tiles[0][0].getColor();
        for (int r=0; r<tiles.length; r++)
            for (int c=0; c<tiles[0].length; c++)
                if (!tiles[r][c].getColor().equals(color))
                    return false;
        return true;
    }
    ...
    private boolean checkerBoard() 
    {
        for (int r=0; r<tiles.length; r++)
            ...
            {
                if ((r + c) % 2 == 0)
                {    if (tiles[r][c].getColor().equals(secondColor))
                        return false;
                }
                else
                {
                    if (tiles[r][c].getColor().equals(firstColor))
                        return false;
                }
            }
        return true;
        ...
        for(int r=0; r<tiles.length; r++){
            for(int c=0; c<tiles[0].length; c++){
                if(r==0 || c==0 || r == tiles.length-1 || c == tiles[0].length-1){
                    if(!tiles[r][c].getColor().equals(firstColor)) return false;
                } else if(!tiles[r][c].getColor().equals(secondColor)) return false;
            }
        }
         
        return true;  // we'll do this
    }
    ...
    private boolean stripes()
    {
        for(int r=0; r<tiles.length; r++){
            for(int c=0; c<tiles[0].length; c++){
                if(c%2 == 0 && tiles[r][c].getColor() != firstColor) return false;
                else if(c%2 != 0 && tiles[r][c].getColor() != secondColor) return false;
            }
        }
